...
def test(message, m):
    ...
    feat = {
        ...
        "has_url": 1
        if any(x in message.lower() for x in ["http", "www", ".com"])
        else 0,
    ...
    e_score_spam = sum(
        get_gaussian_log_prob(
            feat[col], m["spam_stats"]["mean"][col], m["spam_stats"]["var"][col]
        )
        for col in m["eng_cols"]
    )
    e_score_ham = sum(
        get_gaussian_log_prob(
            feat[col], m["ham_stats"]["mean"][col], m["ham_stats"]["var"][col]
        )
        for col in m["eng_cols"]
    )
